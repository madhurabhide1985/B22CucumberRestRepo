...
public class DatatableGETRequestTestSD {
	...
	public void user_validate_following_in_json(DataTable tableObj) throws Throwable {
		List<List<String>> data = tableObj.raw();
		//rows
		int rows = data.size();
		//columns
...
//				
//					String path = data.get(i).get(j);
//					String expectedValue = data.get(i).get(j);
//					
//					System.out.println("Path = "+path);
//					System.out.println("Expected Value = "+expectedValue);
//					
//					String actualLiming = res.body().jsonPath().getString(path);
//					Assert.assertTrue(actualLiming.equals(expectedValue));
//				}
		...
		for(int i = 0; i< rows;i++){
			//for(int j=0;j<cols;j++){
				...
				String expectedValue = data.get(i).get(1);
				String actualLiming = res.body().jsonPath().getString(path);
				
				System.out.println("Path = "+path);
				System.out.println("Expected Value = "+expectedValue);
				System.out.println("Actual Value"+actualLiming);
